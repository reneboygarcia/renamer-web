...
from src.utils.helpers import setup_logger, log_safely, format_show_name
...
class TVShowRenamer:
        ...
        for pattern in patterns:
            ...
            if match:
                try:
                    ...
                    # Handle patterns with only 2 groups (show name and episode)
                    if len(match.groups()) == 2:
                        season = 1  # Default to season 1
                        episode = int(match.group(2))
                        self.logger.debug(f"Extracted episode number: {episode} from pattern with 2 groups")
                    else:
                        season = int(match.group(2))
                        episode = int(match.group(3))
                        self.logger.debug(f"Extracted episode number: {episode} from pattern with 3 groups")
    ...
    def get_show_info(self, show_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            show_data = {
                ...
                "overview": (show_details.get('overview') or '')[:200]  # Limit overview length
            }
    ...
    def get_episode_info(self, show_id: int, season: int, episode: int) -> Optional[Dict]:
        ...
        try:
            ...
            episode_data = {
                ...
                "overview": (details.get('overview') or '')[:200]  # Limit overview length
            }
    ...
    def generate_new_name(
        ...
        try:
            ...
            # Create new filename with consistent casing and spacing
            extension = os.path.splitext(original_name)[1]
            show_name = format_show_name(show_info[0]).strip()
            episode_name = format_show_name(episode_info["name"]).strip()
            
            # Format with consistent spacing around hyphens
    ...
    def sanitize_filename(self, filename: str) -> str:
        """Remove invalid characters and clean up spaces from filename."""
    ...
    def extract_episode_number(self, filename: str) -> Optional[int]:
        ...
        # Patterns to match episode numbers, including specific formats
        patterns = [
        ...
        for pattern in patterns:
            ...
            if match:
                try:
                    episode_num = int(match.group(1))
                    self.logger.debug(f"Found episode number {episode_num} using pattern {pattern}")
                    return episode_num
                except (IndexError, ValueError) as e:
    ...
    #     return {
    #         'avg_api_time': f"{sum(api_times) / len(api_times):.3f}}
